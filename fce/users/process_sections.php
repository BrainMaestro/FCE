        ...
        <div class="main text-center para"> 
            <form action="" method="post">
                ...
                if ($result->num_rows == 0)
                    ...
                    <caption><h3>Sections to Import</h3><h5>Fix the crossed out columns</h5><hr></caption>
                    <thead>
                        <th>CRN</th>
                        <th>Course Code</th>
                        <th>Semester</th>
                    ...
                    for($i = 0; $i < $result->num_rows; $i++) {
                        ...
                        foreach ($row as $value) {
                            ...
                            if (isset($row["$row[error_column]"]) && $value == $row["$row[error_column]"]) {
                                ...
                                echo "<td><input name='theInput' type='text' class='input-element' value='$value'></td>";
                            }
                            else
                                echo "<td>$value</td>";
                        }
                ...
                ?>
                <button class="black-btn" name="submit">Add Sections</button>

            </form>
            <script>
            var inputEl = document.getElementsByName("theInput");

            function getWidthOfInput(i) {
                var tmp = document.createElement("span");
                tmp.className = "input-element tmp-element";
                tmp.innerHTML = inputEl[i].value.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
            ...
            function adjustWidthOfInput() {
                ...
                inputEl[i].style.width = getWidthOfInput(i) + "px";
            }

            adjustWidthOfInput();
            for (var i = 0; i < inputEl.length; i++)
                inputEl[i].onkeyup = adjustWidthOfInput;
