<?php
	...
	if (isset($_POST['key'])){ //Checking to see if any key has been submitted
		...
		$get_key = $mysqli->prepare("SELECT key, key_eval_type, key_crn, given_out FROM AcessKeys 
								WHERE key = '$key'"); //Mysql query to determine if the key exists
		$get_key->bind_param('s', $key);
		$get_key->execute();
		$get_key->store_result();
		$get_key->bind_result($key_value, $key_eval_type, $key_crn, $given_out);
		$get_key->fetch();
